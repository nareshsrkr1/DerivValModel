...
if __name__ == '__main__':
    ...
    try:
        S = Variable(torch.tensor(150.0), requires_grad=True)  # Underlying asset price
        K = Variable(torch.tensor(107.25), requires_grad=True)  # Strike price
        r = Variable(torch.tensor(0.01), requires_grad=True)  # Risk-free interest rate
        T = Variable(torch.tensor(2.0), requires_grad=True)  # Time to maturity in years
        sigma = Variable(torch.tensor(0.8), requires_grad=True)  # Volatility
